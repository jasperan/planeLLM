...
# Generate a longer, more detailed transcript
python podcast_controller.py --topic "Ancient Rome" --transcript-length long

# Process each question individually for even more detailed content
...
# Create podcast transcript (with options for length)
writer = PodcastWriter(transcript_length="medium")  # Options: "short", "medium", "long"
...
generator.generate_podcast(transcript, output_path="./resources/podcast.mp3")
```
...
2. **Lesson Writer** (`lesson_writer.py`): Transforms educational content into podcast format
   ...
   - Offers three transcript length options: short, medium, and long
